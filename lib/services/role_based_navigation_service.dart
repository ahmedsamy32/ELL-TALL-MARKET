...
class UserModel {
  ...
  UserModel({required this.id, required this.type, this.name, this.email});
}

/// User roles for navigation
enum UserRole { admin, captain, store, client, guest, moderator, analyst }
...
enum NavigationPriority { critical, high, normal, low, background }

/// Navigation transition types
enum NavigationTransition { fade, slide, scale, rotation, custom, none }
...
class RoleBasedNavigationServiceEnhanced {
  ...
  Future<Map<String, dynamic>> _performEnhancedNavigation({
    ...
    try {
      // Avoid using BuildContext across async gaps by verifying it's still mounted.
      // (BuildContext has a `mounted` getter as of Flutter 3.7+.)
      if (kDebugMode) {
        AppLogger.info('Starting enhanced navigation for user: ${user.id}');
      }
      ...
      final validation = await _validateNavigationContext(
        context,
        user,
        navigationContext,
      );
      if (!context.mounted) {
        return _createNavigationResult(
          false,
          error: 'Navigation cancelled - context not mounted',
        );
      }
      if (!validation['valid']) {
        return _createNavigationResult(
      ...
      final permissionCheck = await _checkNavigationPermissions(
        user,
        targetRoute,
      );
      if (!context.mounted) {
      ...
      await _recordNavigationEvent(
        NavigationEventType.navigation,
        navigationData,
      );
      if (!context.mounted) {
        return _createNavigationResult(
  ...
  Future<Map<String, dynamic>> _performRoleBasedNavigation({
    ...
    try {
      AppLogger.info('Performing role-based navigation for: $userRole');
      ...
      if (!bypassRestrictions) {
        final hasPermission = await _checkRoleNavigationPermission(
          userRole,
          contextData,
        );
        if (!context.mounted) {
        ...
        if (!hasPermission) {
          return _createNavigationResult(
            false,
            error: 'Insufficient permissions for role navigation',
          );
        }
      }
      ...
      switch (priority) {
        case NavigationPriority.critical:
          return await _executeCriticalNavigation(
            context,
            dashboardRoute,
            roleConfig,
          );
        case NavigationPriority.high:
          return await _executeHighPriorityNavigation(
            context,
            dashboardRoute,
            roleConfig,
          );
        default:
          return await _executeStandardNavigation(
            context,
            dashboardRoute,
            roleConfig,
          );
      ...
      AppLogger.error('Role-based navigation failed', e);
      return _createNavigationResult(false, error: e.toString());
    }
  }
  ...
  Future<Map<String, dynamic>> _handleAdvancedDeepLink({
    ...
    try {
      AppLogger.info('Handling deep link: $deepLink');
      ...
      if (currentUser != null) {
        final hasPermission = await _checkDeepLinkPermission(
          currentUser,
          linkData,
        );
        if (!context.mounted) {
        ...
        if (!hasPermission) {
          return _createNavigationResult(
            false,
            error: 'Insufficient permissions for deep link target',
          );
        }
      }

      // Extract navigation parameters
      final navigationParams = _extractNavigationParameters(
        linkData,
        queryParameters,
      );
      ...
      AppLogger.error('Deep link handling failed', e);
      return _createNavigationResult(false, error: e.toString());
    }
  }
  ...
  Future<void> _recordNavigationEvent(
    ...
    try {
      ...
      AppLogger.error('Failed to record navigation event', e);
    }
  }
    ...
    try {
      ...
      if (userId != null) {
        filteredEvents = events.where((event) {
          final eventData =
              jsonDecode(event['event_data'] ?? '{}') as Map<String, dynamic>;
          return eventData['user_id'] == userId;
        }).toList();
      }

      if (eventTypes != null) {
        final eventTypeStrings = eventTypes
      ...
      AppLogger.error('Failed to get navigation analytics', e);
      return {
        'error': e.toString(),
        'generated_at': DateTime.now().toIso8601String(),
      };
  ...
  Future<void> optimizeNavigationPerformance() async {
    try {
      AppLogger.info('Optimizing navigation performance...');
      ...
      AppLogger.error('Navigation optimization failed', e);
    }
  }
  ...
  Future<Map<String, dynamic>> _validateNavigationContext(
    BuildContext context,
    UserModel user,
    NavigationContext navigationContext,
  ) async => {'valid': true};
  Future<String> _determineTargetRoute(
    UserModel user,
    NavigationContext navigationContext,
    Map<String, dynamic>? additionalData,
  ) async => AppRoutes.home;
  Future<Map<String, dynamic>> _checkNavigationPermissions(
    UserModel user,
    String targetRoute,
  ) async => {'allowed': true};
  Future<void> _performPreNavigationActions(
    BuildContext context,
    UserModel user,
    String targetRoute,
  ) async {}
  Future<Map<String, dynamic>> _executeNavigation({
    required BuildContext context,
    required String targetRoute,
    required NavigationTransition transition,
    required Duration duration,
    required Map<String, dynamic> navigationData,
  }) async => _createNavigationResult(true);
  Future<void> _performPostNavigationActions(
    BuildContext context,
    UserModel user,
    String targetRoute,
  ) async {}
  Future<Map<String, dynamic>> _getRoleConfiguration(UserRole userRole) async =>
      {};
  Map<String, dynamic> _determineNavigationStrategy(
    UserRole userRole,
    Map<String, dynamic>? contextData,
  ) => {};
  Future<bool> _checkRoleNavigationPermission(
    UserRole userRole,
    Map<String, dynamic>? contextData,
  ) async => true;
  Future<Map<String, dynamic>> _executeCriticalNavigation(
    BuildContext context,
    String dashboardRoute,
    Map<String, dynamic> roleConfig,
  ) async => _createNavigationResult(true);
  Future<Map<String, dynamic>> _executeHighPriorityNavigation(
    BuildContext context,
    String dashboardRoute,
    Map<String, dynamic> roleConfig,
  ) async => _createNavigationResult(true);
  Future<Map<String, dynamic>> _executeStandardNavigation(
    BuildContext context,
    String dashboardRoute,
    Map<String, dynamic> roleConfig,
  ) async => _createNavigationResult(true);
  ...
  Future<Map<String, dynamic>> _redirectToLoginWithReturnPath(
    BuildContext context,
    String deepLink,
  ) async => _createNavigationResult(true);
  Future<bool> _checkDeepLinkPermission(
    UserModel currentUser,
    Map<String, dynamic> linkData,
  ) async => true;
  Map<String, dynamic> _extractNavigationParameters(
    Map<String, dynamic> linkData,
    Map<String, dynamic>? queryParameters,
  ) => {};
  Future<Map<String, dynamic>> _executeDeepLinkNavigation({
    required BuildContext context,
    required Map<String, dynamic> linkData,
    required Map<String, dynamic> navigationParams,
    UserModel? currentUser,
  }) async => _createNavigationResult(true);
  Future<void> _recordDeepLinkAnalytics(
    String deepLink,
    Map<String, dynamic> result,
    UserModel? currentUser,
  ) async {}
  Map<String, dynamic> _calculateNavigationAnalytics(
  ...
  Future<void> dispose() async {
    try {
      ...
      if (kDebugMode) {
        AppLogger.info('Navigation service disposed');
      }
    } catch (e) {
