...
class _CouponFormSheet extends StatefulWidget {
  ...
  final MerchantCouponsProvider couponsProvider;
  final String createdBy;

  const _CouponFormSheet({
    this.coupon,
    required this.couponsProvider,
...
class _CouponFormSheetState extends State<_CouponFormSheet> {
  ...
  late final TextEditingController usagePerUserController;
  ...
  late List<QuantityTier> quantityTiers;
  ...
  void initState() {
    super.initState();
    nameController = TextEditingController(text: widget.coupon?.name ?? '');
    codeController = TextEditingController(text: widget.coupon?.code ?? '');
    descriptionController = TextEditingController(
      text: widget.coupon?.description ?? '',
    );
    discountController = TextEditingController(
      text: widget.coupon?.discountValue.toStringAsFixed(2) ?? '',
    );
    minOrderController = TextEditingController(
      text: widget.coupon?.minimumOrderAmount.toString() ?? '0',
    );
    maxDiscountController = TextEditingController(
      text: widget.coupon?.maximumDiscountAmount?.toString() ?? '',
    );
    usageLimitController = TextEditingController(
      text: widget.coupon?.usageLimit?.toString() ?? '',
    );
    usagePerUserController = TextEditingController(
      text: widget.coupon?.usageLimitPerUser.toString() ?? '1',
    );

    selectedType = widget.coupon?.couponType ?? CouponType.percentage;
    validFrom = widget.coupon?.validFrom ?? DateTime.now();
    validUntil = widget.coupon?.validUntil;
    isActive = widget.coupon?.isActive ?? true;

    selectedProductIds = List<String>.from(widget.coupon?.productIds ?? []);
    quantityTiers = List<QuantityTier>.from(widget.coupon?.quantityTiers ?? []);
    activeHoursStart = widget.coupon?.activeHoursStart;
    activeHoursEnd = widget.coupon?.activeHoursEnd;

    if (selectedType == CouponType.productSpecific) {
      _loadStoreProducts();
    }
  }
  ...
  void _loadStoreProducts() {
    if (widget.couponsProvider.storeId == null || loadingProducts) return;
    setState(() {
      loadingProducts = true;
    });
    ProductService.getProductsByStore(
      widget.couponsProvider.storeId!,
      activeOnly: true,
    ).then((products) {
      if (mounted) {
        setState(() {
                  ...
                  child: FocusTraversalGroup(
                    ...
                    child: Column(
                      children: [
                          ...
                          child: DropdownButtonFormField<CouponType>(
                            ...
                            onChanged: (value) {
                              if (value != null) {
                                ...
                                if (value == CouponType.productSpecific &&
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  if (loadingProducts)
                                    ...
                                    Builder(
                                      builder: (context) {
                                        final filtered = productSearchQuery
                                                .isEmpty
                                            ? storeProducts
                                            : storeProducts
                                                .where(
                        ...
                        // ██  شرائح الكمية  ██
                        // ═══════════════════════════════════════
                        if (selectedType == CouponType.tieredQuantity) ...[
                          const SizedBox(height: 8),
                          Card(
                            child: Padding(
                              padding: const EdgeInsets.all(12),
                              child: Column(
                                ...
                                children: [
                                  ...
                                  ...quantityTiers.asMap().entries.map((entry) {
                                    final i = entry.key;
                        ...
                        _DatePickerTile(
                          ...
                          onClear: () => setState(() => validUntil = null),
                        ),
                        SwitchListTile(
                ...
                child: FilledButton(
                  onPressed: () async {
                    ...
                    final input = CouponInput(
                      ...
                      usageLimitPerUser:
                          int.tryParse(usagePerUserController.text.trim()) ?? 1,
                      validFrom: validFrom,
                    ...
                    final success = widget.coupon == null
                        ? await widget.couponsProvider.createCoupon(
                            input: input,
                            createdBy: widget.createdBy,
                          )
                        : await widget.couponsProvider.updateCoupon(
                  ...
                  child: Text(widget.coupon == null ? 'حفظ' : 'تحديث'),
                ),
              ),
            ),
          ],
